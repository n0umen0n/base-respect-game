...
async function main(): Promise<void> {
  ...
  console.log('Contract bytecode length:', SimpleStorage.bytecode.length);
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('is not registered')) {
      ...
      try {
        ...
        await upgrades.forceImport(PROXY_ADDRESS, SimpleStorage);
  ...
  console.log(
    'SimpleStorage proxy address (unchanged):',
